(() => {
    ...
    function rangeInit() {
        ...
        const powerValues = [ document.getElementById("power__value_1"), document.getElementById("power__value_2") ];
        if (powerSlider) {
            powerSlider.getAttribute("data-from");
            powerSlider.getAttribute("data-to");
            ...
            powerSlider.noUiSlider.on("update", (function(values, handle) {
                powerValues[Math.round(handle)].innerHTML = Math.round(values[handle]);
            }));
        }
        const accelerationSlider = document.querySelector("#acceleration");
        const accelerationValues = [ document.getElementById("acceleration__value_1"), document.getElementById("acceleration__value_2") ];
        if (accelerationSlider) {
            accelerationSlider.getAttribute("data-from");
            accelerationSlider.getAttribute("data-to");
            initialize(accelerationSlider, {
                start: [ 2, 10 ],
                connect: true,
                step: 1,
                range: {
            ...
            Math.round();
            accelerationSlider.noUiSlider.on("update", (function(values, handle) {
                accelerationValues[Math.round(handle)].innerHTML = Math.round(values[handle]);
            }));
        }
        const distanceSlider = document.querySelector("#distance");
        const distanceValues = [ document.getElementById("distance__value_1"), document.getElementById("distance__value_2") ];
        if (distanceSlider) {
            distanceSlider.getAttribute("data-from");
            distanceSlider.getAttribute("data-to");
            initialize(distanceSlider, {
                start: [ 120, 1320 ],
                connect: true,
                step: 50,
                range: {
            ...
            distanceSlider.noUiSlider.on("update", (function(values, handle) {
                distanceValues[Math.round(handle)].innerHTML = Math.round(values[handle]);
            }));
        }
        const batterySlider = document.querySelector("#battery");
        const batteryValues = [ document.getElementById("battery__value_1"), document.getElementById("battery__value_2") ];
        if (batterySlider) {
            batterySlider.getAttribute("data-from");
            batterySlider.getAttribute("data-to");
            initialize(batterySlider, {
                start: [ 18, 150 ],
                connect: true,
                step: 1,
                range: {
            ...
            batterySlider.noUiSlider.on("update", (function(values, handle) {
                batteryValues[Math.round(handle)].innerHTML = Math.round(values[handle]);
            }));
        }
    }
    rangeInit();
    ...
    videoContainers.forEach((videoContainer => {
        const videoBtn = videoContainer.querySelector(".js-video-btn");
        const video = videoContainer.querySelector(".js-video");
        const downloadBtn = videoContainer.querySelector(".download-btn");
        videoBtn.addEventListener("click", (function() {
            video.play();
            this.classList.add("hide");
            if (downloadBtn) downloadBtn.classList.add("hide");
        }));
        video.addEventListener("play", (function() {
            this.controls = true;
        }));
    }));
    const openFilter = document.querySelector(".catalog__open-filter");
    if (openFilter) {
